...
function SliderColumnFilter({ column: { render, filterValue, setFilter, preFilteredRows, id } }) {
...
function NumberRangeColumnFilter({ column: { filterValue = [], render, preFilteredRows, setFilter, id } }) {
  const [min, max] = React.useMemo(() => getMinMax(preFilteredRows, id), [id, preFilteredRows])
...
const App = () => {
  ...
  return (
    <Page>
      <CssBaseline />
      <Table name={'testTable'} columns={columns} data={data} onAdd={dummy} onEdit={dummy} onDelete={dummy} />
    </Page>
  )
}
